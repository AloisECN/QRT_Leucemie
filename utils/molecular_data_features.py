...
class MolecularDataFeature:
    ...
    def is_high_risk(self, ref, alt, protein_change):
        ...
        if protein_change is None or pd.isna(protein_change):
            ...
            prot_risky = any(
                risk in str(protein_change) for risk in risky_protein_signatures
            )
    ...
    def process(self, df):
        ...
        df["HIGH_RISK"] = df.apply(
            lambda x: self.is_high_risk(x["REF"], x["ALT"], x["PROTEIN_CHANGE"]), axis=1
        )
        df["HOTSPOTS"] = df.apply(
            lambda x: self.has_hotspot_mutation(x["PROTEIN_CHANGE"]), axis=1
        )
        df["EFFECT_LEVEL"] = df.apply(
            lambda x: self.classify_impact(x["EFFECT"]), axis=1
        )
        ...
        gene_indicators = gene_indicators.reindex(
            columns=self.genes_of_interest, fill_value=0
        )
